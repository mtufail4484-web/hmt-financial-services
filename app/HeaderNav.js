...
const navLinks = [
  ...
  { href: "https://library.hmtfinancialservices.com", label: "📚 Digital Library", isExternal: true },
...
export default function HeaderNav() {
  ...
  return (
    <>
      <header className="w-full bg-[#0b2c5f] text-white shadow-lg print:hidden">
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-3 px-3 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          ...
          <nav aria-label="Main navigation" className="grid grid-cols-2 gap-2 text-center text-[12px] font-bold sm:flex sm:items-center sm:gap-2 sm:text-sm">
            {navLinks.map((link) => {
              const active = !link.isExternal && isActiveRoute(pathname, link.href);
              ...
              const idleClass = link.featured
                ...
                : link.isExternal
                ? "bg-purple-600/90 text-white hover:bg-purple-500 border border-purple-400/40"
              ...
              if (link.isExternal) {
                return (
                  <a
                    ...
                    className={`rounded-xl px-2 py-2 leading-tight transition ${idleClass} sm:px-3 font-bold`}
                  >
                    {link.label}
                  </a>
                );
      ...
      {/* SITE-WIDE FLOATING HMT AI TUTOR WIDGET */}
      <AiTutorWidget />
    </>
  );
}
