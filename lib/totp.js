...
export async function verifyTOTP(token, secret, timeStepSeconds = 30) {
  ...
  const now = Date.now();
  const windows = [-4, -3, -2, -1, 0, 1, 2, 3, 4]; // check +/- 4 time steps (2 mins) to handle device clock drift
...
/**
 * Trigger download of Emergency Backup Recovery Codes as a text file.
 */
export function downloadBackupCodesFile(codes = [], accountEmail = "m.tufailkhan12335@gmail.com") {
  if (!Array.isArray(codes) || codes.length === 0) return;
  const content = `=====================================================
HMT SUCCESS ACADEMY — EMERGENCY 2FA BACKUP CODES
=====================================================
Account: ${accountEmail}
Generated: ${new Date().toLocaleString()}

Keep these 10 one-time emergency recovery codes in a safe place.
If you lose access to Google Authenticator, you can use any of
these codes to log into your admin account. Each code can be used ONCE.

${codes.map((c, i) => ` ${i + 1}. ${c}`).join("\n")}

=====================================================
Official Site: https://www.hmtfinancialservices.com
=====================================================`;
  ...
  link.href = url;
  link.download = `HMT-2FA-Emergency-Backup-Codes.txt`;
